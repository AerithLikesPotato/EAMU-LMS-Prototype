...
async function saveTask() {
    ...
    for (let i = 1; i <= questionCount; i++) {
        const questionElem = document.getElementById(`question_${i}`);
        if (!questionElem) continue;

        const questionText = document.querySelector(`[name="question_${i}_text"]`).value;
        const correctValue = document.querySelector(`[name="correct_${i}"]:checked`)?.value;

        if (questionText && correctValue !== undefined) {
            const options = [];
            document.querySelectorAll(`[name^="option_${i}_"]`).forEach(opt => {
