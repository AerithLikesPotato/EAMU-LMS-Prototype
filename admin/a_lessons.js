...
grid.innerHTML = data.data.map((lesson, idx) => {
            const title    = lesson.Lesson_Title || 'Untitled Lesson';
            const course   = lesson.Course_Title  || '';
            const duration = lesson.Lesson_Duration || '';
            const hasVideo = !!lesson.Lesson_Video_URL;
            const order    = lesson.Lesson_Order || (idx + 1);

            const thumbContent = hasVideo
                ? '<div class="play-icon"><i class="fas fa-play"></i></div>'
                : '<i class="fas fa-file-alt thumb-doc-icon"></i>';

            const durationBadge = duration
                ? '<span class="lesson-duration-badge"><i class="fas fa-clock" style="margin-right:3px;font-size:0.65rem;"></i>' + duration + '</span>'
                : '';

            const coursePill = course
                ? '<span class="lesson-card-course"><i class="fas fa-book"></i>' + course + '</span>'
                : '';

            const watchBtn = hasVideo
                ? '<a href="' + lesson.Lesson_Video_URL + '" target="_blank" class="la-btn la-watch"><i class="fas fa-play"></i> Watch</a>'
                : '';

            return '<div class="lesson-mgmt-card">' +
                '<div class="lesson-card-thumb">' +
                    '<span class="lesson-order-badge">#' + order + '</span>' +
                    thumbContent +
                    durationBadge +
                '</div>' +
                ...
                '<div class="lesson-card-actions">' +
                    watchBtn +
                    '<button class="la-btn la-edit" onclick="editLesson(' + lesson.Lesson_ID + ')"><i class="fas fa-pencil-alt"></i> Edit</button>' +
                    '<button class="la-btn la-delete" onclick="deleteLesson(' + lesson.Lesson_ID + ')"><i class="fas fa-trash-alt"></i> Delete</button>' +
                '</div>' +
            '</div>';
        }).join('');
    } catch (err) {
